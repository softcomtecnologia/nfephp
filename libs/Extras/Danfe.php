...
class Danfe extends CommonNFePHP implements DocumentoNFePHP
{
    ...
    public function montaDANFE(
        ...
        $hfooter = -15;// para rodape (default = 5)
        $hCabecItens = 8;//cabeçalho dos itens (default = 4)
        ...
        while ($i < $this->det->length) {
            ...
            $hUsado += round(($numlinhas * $this->pdf->fontSize) + ($numlinhas * 1), 2);
            if ($hUsado > $hDispo) {
                $totPag++;
                $hDispo = $hDispo2;
                $hUsado = $hCabecItens;
    ...
    protected function pCabecalhoDANFE($x = 0, $y = 0, $pag = '1', $totPag = '1')
    {
        ...
        $x += $w;
        $w = ($maxW-(2*$w));
        $doc = !empty($this->emit->getElementsByTagName("CNPJ")->item(0)->nodeValue) ? "CNPJ" : "CPF";
        $texto = $doc;
        $aFont = array('font'=>$this->fontePadrao, 'size'=>6, 'style'=>'');
        $this->pTextBox($x, $y, $w, $h, $texto, $aFont, 'T', 'L', 1, '');
        $texto = $this->emit->getElementsByTagName($doc)->item(0)->nodeValue;
        $texto = $this->pFormat($texto, ($doc === "CNPJ" ? "##.###.###/####-##" : "###.###.###-##"));
        $aFont = array('font'=>$this->fontePadrao, 'size'=>10, 'style'=>'B');
        $this->pTextBox($x, $y, $w, $h, $texto, $aFont, 'B', 'C', 0, '');
    ...
    protected function pGeraInformacoesDasNotasReferenciadas()
    {
        ...
        foreach ($nfRefs as $key => $nfRef) {
            ...
            if($key >= 5){
                $lastFormaRefArray = explode(":", $lastFormaRef);
                return $saida . "$lastFormaRefArray[0]: ...";
            }
    ...
    public function setCartaCorrecao($carta)
    {
        $this->cartaCorrecao = $carta;
